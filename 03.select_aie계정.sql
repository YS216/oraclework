...
--------------------------------------------------------------------------------------------------------------
/*
    SUPER : 문자열에서 특정 문자열을 추출하여 반환 (CHARACHER)
    SUBSTR('문자열', POSITION, [LENGTH])
     - POSITION :문자열을 추출할 시작위치 INDEX
     - LENGTH : 추출할 문자 개수(생략시 맨마지막까지 추출)
*/
SELECT SUBSTR('OGACLEHTMLCSS',7)FROM DUAL; -- 결과 : HTMLCSS
SELECT SUBSTR('OGACLEHTMLCSS',7, 4)FROM DUAL; -- 결과 : HTML
SELECT SUBSTR('OGACLEHTMLCSS',1, 6)FROM DUAL; -- 결과 : ORACLE
SELECT SUBSTR('OGACLEHTMLCSS',-7, 4)FROM DUAL; -- 결과 : HTML (INDEX가 음수이면 뒤에서부터 카운트)

-- 주민번호에서 성별만 추출하여 주민번호, 사원명, 성별 조회
